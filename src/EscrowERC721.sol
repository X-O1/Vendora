...
contract EscrowERC721 {
    ...
    /** MAPPINGS */
    mapping(address => mapping(address => uint256)) public s_userBalanceERC721;
    mapping(address => mapping(address => uint256)) private s_offeredERC721;
    mapping(address => mapping(address => uint256)) private s_requestedERC721;
    ...
    // ADD ERC721 REQUEST TO TRADE TERMS
    function requestERC721(
        address collection,
        uint256 tokenId
    ) external onlySeller creatingTermsRequires {
        require(tokenId > 0, "Must choose a valid tokenId");

        if (s_requestedERC721[s_seller][collection] == 0) {
            s_numOfAssetsInTradeTerms++;
        }

        s_requestedERC721[s_seller][collection] += tokenId;
    }

    // DELETE ERC721 REQUEST IN TRADE TERMS
    function deleteRequestedERC721(
        address collection,
        uint256 tokenId
    ) external onlySeller creatingTermsRequires {
        require(
            s_requestedERC721[s_seller][collection] > 0,
            "Nothing to delete"
        );
        require(
            s_requestedERC721[s_seller][collection] >= tokenId,
    ...
    // ADD ERC721 OFFER TO TRADE TERMS
    function offerERC721(
        address collection,
        uint256 tokenId
    ) external onlySeller creatingTermsRequires {
        require(tokenId > 0, "Must choose an amount greater than zero");

        if (s_offeredERC721[s_seller][collection] == 0) {
            s_numOfAssetsInTradeTerms++;
        }

        s_offeredERC721[s_seller][collection] += tokenId;
    }

    // DELETE ERC721 OFFER IN TRADE TERMS
    function deleteOfferedERC721(
        address collection,
        uint256 tokenId
    ) external onlySeller creatingTermsRequires {
        require(s_offeredERC721[s_seller][collection] > 0, "Nothing to delete");
        require(
            s_offeredERC721[s_seller][collection] >= tokenId,
            "Trying to delete more than available in trade terms"
        );
    ...
    function depositERC20Seller(
        ...
        nft.safeTransfer(s_seller, address(this), tokenId);

        // Update balances
        s_userBalanceERC721[s_seller][collection] += tokenId;

        // Check if seller made all deposits offered in trade terms
        if (
            s_userBalanceERC721[s_seller][collection] ==
            s_offeredERC721[s_seller][collection]
        ) {
    ...
    function depositERC20Buyer(
        ...
        // Update balances
        s_userBalanceERC721[s_buyer][token] += amount;

        // Check if buyer made all deposits requested in trade terms
        if (
            s_userBalanceERC721[s_buyer][token] ==
            s_requestedERC721[s_seller][token]
        ) {
            s_numOfAssetsDeposited++;
    ...
    function withdrawBeforeTermsAreMetSeller(
        ...
        require(
            s_userBalanceERC721[s_seller][token] >= amount,
            "Insufficient funds"
        );
        ...
        if (s_numOfAssetsDeposited != 0) {
            if (
                s_userBalanceERC721[s_seller][token] !=
                s_offeredERC721[s_seller][token]
            ) {
                s_numOfAssetsDeposited--;
            }
    ...
    function getUserDepositBalances(
        ...
        return s_userBalanceERC721[user][token];
    }
    ...
    function getRequestedERC20Tokens(
        ...
        return s_requestedERC721[buyer][token];
    }
    ...
    function getOfferedERC20Tokens(
        ...
        return s_offeredERC721[seller][token];
    }
    ...
    function getIfOtherUserOwnsThisAsset(
        ...
        if (s_userBalanceERC721[user][token] > 0) {
