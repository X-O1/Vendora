import { finalizeTermsDiv, offeredTermsErc1155s, offeredTermsErc20s, offeredTermsErc721s, offeredTermsEth, requestedTermsErc1155s, requestedTermsErc20s, requestedTermsErc721s, requestedTermsEth, } from "./FrontEndElements.js";
...
const createWantedAssetList = async (wantedAssets) => {
    try {
        wantedAssets === null || wantedAssets === void 0 ? void 0 : wantedAssets.forEach((token) => {
            const requestedTermsDiv = createTermELements();
            if (wantedAssets === wantedErc721s) {
                requestedTermsErc721s.appendChild(requestedTermsDiv.selectedTermAssetDiv);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetTokenIdTitle);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetTokenId);
                if (finalizeTermsDiv.style.height === "50%") {
                    requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.deleteAssetButton);
                }
            }
            else if (wantedAssets === wantedErc1155s) {
                requestedTermsErc1155s.appendChild(requestedTermsDiv.selectedTermAssetDiv);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetSymbol);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetTokenIdTitle);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetTokenId);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetAmountTitle);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetAmount);
                if (finalizeTermsDiv.style.height === "50%") {
                    requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.deleteAssetButton);
                }
            }
            else if (wantedAssets === wantedErc20s) {
                requestedTermsErc20s.appendChild(requestedTermsDiv.selectedTermAssetDiv);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetAmountTitle);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetAmount);
                if (finalizeTermsDiv.style.height === "50%") {
                    requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.deleteAssetButton);
                ...
                requestedTermsEth.appendChild(requestedTermsDiv.selectedTermAssetDiv);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetAmountTitle);
                requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.termAssetAmount);
                if (finalizeTermsDiv.style.height === "50%") {
                    requestedTermsDiv.selectedTermAssetDiv.appendChild(requestedTermsDiv.deleteAssetButton);
            ...
            else if (wantedAssets === wantedErc1155s && wantedAssets.length !== 0) {
                requestedTermsErc1155s.style.display = "block";
            }
            else if (wantedAssets === wantedErc20s && wantedAssets.length !== 0) {
            ...
            requestedTermsDiv.deleteAssetButton.addEventListener("click", () => {
                if (token.symbol === requestedTermsDiv.termAssetSymbol.innerText) {
                    ...
                    if (token.tokenId === requestedTermsDiv.termAssetTokenId.innerText) {
                        const index = wantedAssets.indexOf(token);
                        index !== -1 ? wantedAssets.splice(index, 1) : null;
                        setItem("wantedErc721s", wantedErc721s);
                        setItem("wantedErc1155s", wantedErc1155s);
                        if (wantedAssets === wantedErc1155s ||
...
const createOfferedAssetList = async (offeredAssets) => {
    try {
        offeredAssets === null || offeredAssets === void 0 ? void 0 : offeredAssets.forEach((token) => {
            const offeredTermsDiv = createTermELements();
            if (offeredAssets === offeredErc721s) {
                offeredTermsErc721s.appendChild(offeredTermsDiv.selectedTermAssetDiv);
                offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.termAssetTokenIdTitle);
                offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.termAssetTokenId);
                if (finalizeTermsDiv.style.height === "50%") {
                    offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.deleteAssetButton);
                }
            }
            else if (offeredAssets === offeredErc1155s) {
                offeredTermsErc1155s.appendChild(offeredTermsDiv.selectedTermAssetDiv);
                offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.termAssetSymbol);
                offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.termAssetTokenIdTitle);
                offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.termAssetTokenId);
                offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.termAssetAmountTitle);
                offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.termAssetAmount);
                if (finalizeTermsDiv.style.height === "50%") {
                    offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.deleteAssetButton);
                }
            }
            else if (offeredAssets === offeredErc20s) {
                offeredTermsErc20s.appendChild(offeredTermsDiv.selectedTermAssetDiv);
                offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.termAssetAmountTitle);
                offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.termAssetAmount);
                if (finalizeTermsDiv.style.height === "50%") {
                    offeredTermsDiv.selectedTermAssetDiv.appendChild(offeredTermsDiv.deleteAssetButton);
            ...
            else if (offeredAssets === offeredEth && offeredAssets.length !== 0) {
            ...
            offeredTermsDiv.deleteAssetButton.addEventListener("click", () => {
                if (token.symbol === offeredTermsDiv.termAssetSymbol.innerText) {
                    ...
                    if (offeredAssets === offeredErc20s || offeredAssets === offeredEth) {
                    ...
                    if (token.tokenId === offeredTermsDiv.termAssetTokenId.innerText) {
                        const index = offeredAssets.indexOf(token);
                        index !== -1 ? offeredAssets.splice(index, 1) : null;
                        setItem("offeredErc721s", offeredErc721s);
                        setItem("offeredErc1155s", offeredErc1155s);
                        if (offeredAssets === offeredErc1155s ||
